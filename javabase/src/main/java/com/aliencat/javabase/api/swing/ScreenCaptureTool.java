 ...
 * 截图工具类
 */
public class ScreenCaptureTool extends Frame implements MouseListener, MouseMotionListener, KeyListener {

    private static final long serialVersionUID = 1L;
    private static final String title = "Java屏幕截图工具";
    ...
    private static final int DOUBLE_CLICK_SAVE_CODE = 0x11; //双击保存
    private static final int DOUBLE_CLICK_COPY_CODE = 0x12;  //双击复制
    private static final int PRESS_ENTER_SAVE_CODE = 0x21;   //回车保存
    private static final int PRESS_ENTER_COPY_CODE = 0x22;   //回车复制
    private static final int EXIT_AFTER_SAVE_CODE = 0x01;    //保存后推出
    private static final int EXIT_AFTER_COPY_CODE = 0x02;    //复制后退出
    ...
    private class MyTakePicture implements ActionListener {
        // 保存图片
        public void actionPerformed(ActionEvent e) {
            FileDialog fileDialog = new FileDialog(ScreenCaptureTool.this, "保存截图", FileDialog.SAVE);
            ...
            if (dir != null && !"".equals(dir) && (fileName == null || "".equals(fileName))) {
                JOptionPane.showMessageDialog(ScreenCaptureTool.this, "Error file!", "Error",
            ...
            if (dirFile.exists()) {
                if (!dirFile.isDirectory()) {
                    JOptionPane.showMessageDialog(ScreenCaptureTool.this, "\"" + dir + "\" is not a directory!",
                            "Error", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                int ret = JOptionPane.showConfirmDialog(ScreenCaptureTool.this, "\"" + dir
