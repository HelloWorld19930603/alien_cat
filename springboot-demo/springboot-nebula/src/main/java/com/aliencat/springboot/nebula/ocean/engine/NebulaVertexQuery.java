...
public class NebulaVertexQuery implements VertexQuery {
    ...
    @Override
    public VertexQuery fetchPropOn(Class clazz, Set<String> vertexIds) {
    ...
    @Override
    public VertexQuery getTagByParam(Class clazz, String param) {
        GraphVertexType graphVertexType = graphTypeManager.getGraphVertexType(clazz);
        String vertexName = graphVertexType.getVertexName();
        sqlBuilder.append("LOOKUP ON ").append(vertexName).append(" WHERE ").append(param);
        return this;
    }

    @Override
    public VertexQuery matchTagByVertexIds(Class clazz, List<String> vertexIds) {
        GraphVertexType graphVertexType = graphTypeManager.getGraphVertexType(clazz);
        sqlBuilder.append(" MATCH (v) WHERE id(v) IN [");
//        NebulaQueryUtils.appendVertexId(graphVertexType, sqlBuilder, vertexIds);
        sqlBuilder.deleteCharAt(sqlBuilder.length() - 1);
        sqlBuilder.append(" ] RETURN v;");
